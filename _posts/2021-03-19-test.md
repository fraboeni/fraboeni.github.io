...
Unfortunately, when dealing with a malicious central server, even (3) cannot prevent our attacks. This is because, as we mentioned in the introduction, SA in FL can be eluded as shown by [[Pasquini et al.](https://arxiv.org/pdf/2111.07380.pdf)] when the server sends out different models to different users Alternatively, a malicious server can directly control a very small fraction of devices that participate in the FL protocol, select them to participate in the SA together with one target user’s device, and make the controlled devices contribute only zero gradients to extract the target user’s private data from the aggregated gradients. In practice, companies that deploy FL on end-user devices can almost always [introduce as many “evil clone” (sybil) devices as they please](https://arxiv.org/pdf/2009.10031.pdf), to effectively circumvent any SA which assumes a fraction of honest participants.
...
Use shared models with architectures where our attack is more difficult (e.g. containing aggressive lossy layers such as Dropout or Pooling).
